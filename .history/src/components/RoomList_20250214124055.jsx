  ...
  useEffect(() => {
    const updateRoomAvailability = async () => {
      ...
      bookingsQuerySnapshot.forEach(async (bookingDoc) => {
        ...
        // Check if booking data is valid and has a checkOutDate and roomType
        if (booking && booking.checkOutDate && booking.roomType) {
          const checkOutDate = new Date(booking.checkOutDate);

          // Check if the checkout date is valid
          if (!isNaN(checkOutDate)) {
            if (checkOutDate < today) {
              // Try to find the roomId based on the roomType
              const roomsSnapshot = await getDocs(collection(db, "rooms"));
              const room = roomsSnapshot.docs.find(
                (doc) => doc.data().type === booking.roomType
              );
              ...
              if (room) {
                ...
                console.error("Room not found for type:", booking.roomType);
              }
            }
          } else {
          ...
          console.warn("Missing booking, checkOutDate, or roomType:", booking);
        }
      });
    };
