...
const AdminDashboard = () => {
  ...
  const handleUpdateRoom = async () => {
    if (editingRoom) {
      try {
        ...
        // Dispatch action to update all rooms of the same type
        dispatch(updateRoomImagesByType(updatedRoom.type, updatedRoom.images));
        setEditingRoom(null);
      } catch (error) {
  ...
  return (
    <DashboardWrapper>
              ...
              {room.images && room.images.length > 0 && (
                <div>
                  {room.images.map((image, index) => (
                    <img
                      key={`${room.id}-${index}`} // Ensures a unique key for each image
                      src={image}
      ...
      {/* Go to Reservations */}
      <ReservationsButton onClick={handleGoToReservations}>
        View Reservations
      </ReservationsButton>
    </DashboardWrapper>
  );
